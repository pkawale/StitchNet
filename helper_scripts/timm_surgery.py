...
def split_resnet(model: ResNet, split_idx: int):
    ...
    if split_idx == 0:
        ...
        raise ValueError(
            f"Invalid split index {split_idx} for ResNet with {len(blocks)} blocks"
        )
...
def get_splittable_range(model_name: str):
    """
    Get the range of valid split indices for the given model, inclusive. Returns min_idx, integers.
    """
